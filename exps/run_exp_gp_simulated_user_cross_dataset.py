...
from threads.GPSimulatedUserDoubleDatasetExpsExecutor import GPSimulatedDoubleDatasetUserExpsExecutor
...
if __name__ == "__main__":
    ...
    for split_seed in [40, 41, 42]:
        for data_path_files in [("heating", "boston_restricted"), ("heating", "boston_restricted")]:
            ...
            dataset0 = SklearnDatasetPreProcessor.load_data(data_path_files[0], rng_seed=split_seed, previous_seed=100,
                                                            path_dict=path_dict)
            dataset1 = SklearnDatasetPreProcessor.load_data(data_path_files[1], rng_seed=split_seed, previous_seed=100,
                                                            path_dict=path_dict)
            datasets = (dataset0, dataset1)

            data_generator: DatasetGenerator = ExpsUtil.create_dataset_generator_with_warmup(folder_name,
                                                                                             data_path_files[0],
                                                                                             structure, ground_truths)
            runner: GPSimulatedDoubleDatasetUserExpsExecutor = GPSimulatedDoubleDatasetUserExpsExecutor(folder_name,
                                                                                                        data_path_files,
