...
if __name__ == "__main__":
    random.seed(42)
    np.random.seed(42)
    df = pd.read_csv("ale.csv")  # change to your local path in which the dataset is located
    df = df.drop(["sd_ale"], axis=1, inplace=False)
    print(df.head())
    ...
    train_y, val_y, test_y = train["ale"].copy(), val["ale"].copy(), test["ale"].copy()
    train_X, val_X, test_X = train.drop(["ale"], axis=1, inplace=False), val.drop(["ale"], axis=1,
                                                                                   inplace=False), test.drop(["ale"],
                                                                                                             axis=1,
                                                                                                             inplace=False)
    ...
    PicklePersist.compress_pickle("ale", {"training": (X_train, y_train),
