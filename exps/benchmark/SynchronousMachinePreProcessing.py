...
if __name__ == "__main__":
    random.seed(42)
    np.random.seed(42)
    df = pd.read_csv("synchronous machine.csv", sep=";")  # change to your local path in which the dataset is located
    print(df.head())
    ...
    train_y, val_y, test_y = train["If"].copy(), val["If"].copy(), test["If"].copy()
    train_X, val_X, test_X = train.drop(["If"], axis=1, inplace=False), val.drop(["If"], axis=1,
                                                                                   inplace=False), test.drop(["If"],
                                                                                                             axis=1,
                                                                                                             inplace=False)
    ...
    PicklePersist.compress_pickle("synchronousmachine", {"training": (X_train, y_train),
