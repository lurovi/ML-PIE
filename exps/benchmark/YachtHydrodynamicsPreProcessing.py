...
if __name__ == "__main__":
    random.seed(42)
    np.random.seed(42)
    df = pd.read_table("yacht_hydrodynamics.data", header=None, sep=" ")  # change to your local path in which the dataset is located
    print(df.head())
    ...
    train_y, val_y, test_y = train[6].copy(), val[6].copy(), test[6].copy()
    train_X, val_X, test_X = train.drop([6], axis=1, inplace=False), val.drop([6], axis=1,
                                                                                   inplace=False), test.drop([6],
                                                                                                             axis=1,
                                                                                                             inplace=False)
    ...
    PicklePersist.compress_pickle("yachthydrodynamics", {"training": (X_train, y_train),
