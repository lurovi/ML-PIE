...
class UncertaintyChooserDistanceEmbeddings(PairChooser):
    ...
    def sample(self, queue: Set[Node], encoder: TreeEncoder = None, trainer: Trainer = None) -> List[Tuple[Node, Node]]:
        ...
        scaling_uncert = self.__normalization_func(uncertainty)
        ...
        for _ in range(self.get_n_pairs()):
            ...
            scaling_dist = self.__normalization_func(dist)
            l2_uncertainty = [uncertainty[i] / float(scaling_uncert) + self.__lambda_coeff * (dist[i] / float(scaling_dist))
                              for i in range(len(uncertainty))]
            _, ind_points_l2 = Sort.heapsort(l2_uncertainty, lambda a, b: a < b, inplace=False, reverse=True)
            count = 0
            i = 0
            while count < 1 and i < len(ind_points_l2):
                idx = ind_points_l2[i]
                curr_tree = curr_queue[idx]
                if idx not in already_seen_indexes:
                    already_seen_indexes.append(idx)
    ...
    def get_string_repr(self) -> str:
        return "Uncertainty Sampler L2 Distances " + (self.__normalization_func_name[0].upper() + self.__normalization_func_name[1:]) + " Norm"
