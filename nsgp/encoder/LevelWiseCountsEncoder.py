...
class LevelWiseCountsEncoder(TreeEncoder):
    def __init__(self, structure: TreeStructure, additional_properties: bool = True, seed: int = None):
        ...
        data = [self.get_structure().generate_tree() for _ in range(10 ** 6)]
    ...
    def size(self) -> int:
        return int((self.get_structure().get_number_of_operators() + self.get_structure().get_number_of_features() + 1) * self.get_structure().get_number_of_layers()) + (3 if self.__additional_properties else 0)
