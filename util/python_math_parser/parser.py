...
This implementation of parser try to achieve:
    the readability and simplicity of Pascal's.
    ability to parse common math expressions, including vector, matrix, set, etc.
...
class Parser:
    ...
    def equality(self):
        ...
        rel_op = AtomOp(self.current_token, is_func=True)
        self.forward()
        right = self.expr()
        node = RelationOp(left, rel_op, right)
        return node
