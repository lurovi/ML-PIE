def heapify(arr, ind, comparator, n, i, reverse):
    largest = i
    l = 2 * i + 1
    r = 2 * i + 2
    if not(reverse):
    ...
    if largest != i:
        ...
        heapify(arr, ind, comparator, n, largest, reverse)
...
def heapsort(v, comparator, inplace=False, reverse=False):
    ...
    for i in range(n // 2 - 1, -1, -1):
        heapify(arr, ind, comparator, n, i, reverse)

    for i in range(n - 1, 0, -1):
        arr[i], arr[0] = arr[0], arr[i]
        ind[i], ind[0] = ind[0], ind[i]
        heapify(arr, ind, comparator, i, 0, reverse)
