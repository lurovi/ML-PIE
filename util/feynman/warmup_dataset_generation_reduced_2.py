...
def complexify(tree: Node) -> Node:
    ...
    structure_feymann = TreeStructure(feymann_operators, 6, 3,
...
if __name__ == "__main__":
    ...
    for formula in ast_formulae:
        jump = False
        formu = formula.replace("pi", str(round(math.pi, 2)))
        parsed_tree = tree_from_prefix_repr(formu)
        if parsed_tree.get_height() > 3:
            jump = True
        ...
        for o in oper:
            if o.startswith("arc") or o == "tanh" or o == "cos" or o == "sin" or o == "**" or o == "**2" or o == "sqrt" or o == "exp" or o == "u-":
                jump = True
        if jump:
            continue
    ...
    d.to_csv(data_dir + "FeynmanEquationsWarmUp2.csv", index=False)
