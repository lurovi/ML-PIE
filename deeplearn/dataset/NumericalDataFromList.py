...
class NumericalDataFromList(Dataset):
    def __init__(self, X: List[List[float]], y: List[float], scaler: Any = None):
        self.__X = X  # matrix of float values
        self.__y = y  # list
        self.__scaler = scaler  # e.g., StandardScaler(), already fitted to the training data
        if not(scaler is None):
            self.__X = scaler.transform(np.array(self.__X))  # transform data
            self.__X = self.__X.tolist()
        self.__X = torch.tensor(self.__X).float()
        self.__y = torch.tensor(self.__y).float()
    ...
    def subset(self, indexes: List[int]) -> NumericalDataFromList:
        X, y = [], []
        for i in indexes:
            if not (0 <= i < len(self)):
            ...
            y.append(label.item())
        return NumericalDataFromList(X, y)
    ...
    def remove_ground_truth_duplicates(self) -> NumericalDataFromList:
        new_X, new_y = [], []
        for i in range(len(self)):
            is_dupl = False
            j = i+1
        ...
        return NumericalDataFromList(new_X, new_y, scaler=None)
